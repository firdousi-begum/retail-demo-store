...
while [ "$1" ]; do
    if [[ $1 == "--"* ]]; then
        case "$1" in
            --private-s3)
                echo "Received a \"--private-s3\" flag. This is now the default behavior, so you don't have to use this parameter anymore."
                ;;
            --only-cfn-template)
            ...
            --skip-virtualenv)
                ...
                ;;
            *)
                echo "Received an unknown flag \"$1\""
                exit 1
                ;;
        esac
    else
...
if [ "$only_cfn_template" = false ]; then
    ...
    zip a -r retaildemostore-source.zip ./src/ -x!*.DS_Store -x!*__pycache__* -x!*/aws-lambda/* -x!*/node_modules/* -x!*.zip -x!*/venv* -x!*/.venv* -x!*/dist/* -x!*/dist-layer0/*
    ...
    for function in ./src/aws-lambda/*/
        do
            # Check if the directory is pinpoint-sms-alerts
            # if  [ "$(basename $function)" == "amazon-pay-signinhg" ]; then
